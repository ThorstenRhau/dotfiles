...
font-variation-bold-italic = GRAD = $grade"
          ...
          case "$*" in
            "status server")
              [[ -f "$herdr_dir/config.toml" &&
                "$(<"$herdr_dir/config.toml")" == *"$expected_theme" ]] || exit 92
            ...
            "server reload-config")
              [[ -f "$herdr_dir/config.toml" &&
                "$(<"$herdr_dir/config.toml")" == *"$expected_theme" ]] || exit 93
    ...
    def test_herdr_generation_family_switching_and_reload_order(self):
        self.run_zsh(
            ...
            expected_theme="$(<"$herdr_dir/themes/token.toml")"
            _apply_appearance
            expected="$header
...
$(<"$herdr_dir/themes/token.toml")"
            ...
            expected_theme="$(<"$herdr_dir/themes/token-flint.toml")"
            herdr_running=true
            _apply_appearance
            [[ "$(<"$herdr_events")" == $'status\nstatus\nreload' ]] || exit 5
            [[ "$(<"$config")" == *"$expected_theme" ]] || exit 6
        """,
            HERDR_SOURCE,
        )
